...
<div class="container-xxl">
    ...
    <tr>      
       ...
       @else
        ...
        <td class="text-right">
          <button class="btn btn-success" title="View Request" onclick="openNewWindow({{ $it->id }})">
        ...
        @if($it->status->status_code != 'approved' && $it->status->status_code != 'Rejected' && auth()->user()->name == $it->status->approver)
        @if($it->status->status_code != 'verified')
        
        <a href="/edit/{{ $it->id }}"><button class="btn btn-primary" title="Edit Request">
        ...
        <!-- verify btn
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#verifyModal //{ $it->id }">
    ...
    -->
      ...
      @if($it->status->status_code != 'approved')
